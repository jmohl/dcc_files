...
tcol <- function(col, alpha = 1) {x <- col2rgb(col)/255; return(rgb(x[1],x[2],x[3],alpha = alpha))}

#### Wrapper function for real data runs ####

fitter.fn <- function(triplet, triplet.meta, start.time = 0, end.time = 1000, bw = 25, on.reward = TRUE, go.by.soff = TRUE, save.figure = FALSE, save.out = FALSE, tilt = TRUE, data.path = "http://www2.stat.duke.edu/~st118/Jenni/STCodes/", local.pull = FALSE, save.path = "./", ...){
...
plot.neurodyn <- function(fit, add.prior = TRUE, true.alphas = NULL, tilt.prior = FALSE, mesh.tilt = 0.1, nprior = fit$mcmc["nsamp"]){
  ...
  alpha.post.pred <- fit$alpha.pred
  lambda_A_POST <- fit$lambda.A
  lambda_B_POST <- fit$lambda.B
  minmax.post.pred <- apply(alpha.post.pred, 1, function(a) diff(range(a)))
  ...
  # make plot of alpha means
  #par(mfrow = c(3,2))
  
  ## Estimated alpha
  alpha.post.mean <- apply(alpha.post, c(2,3), mean)
  mean_df <-  data.frame(Time = bin.mids, alpha.post.mean)
  
  mean_df <-  melt(mean_df, id.vars = "Time")
  
  p1 <-  ggplot(mean_df) +
    geom_line(aes(x = Time, y = value,
                  group = variable, 
                  colour = variable), alpha = 1) + 
  ...
  if(length(true.alphas) > 0){
    # add true lines, if given
    alphas <- synth.data$alphas
    mids <- synth.data$time.pts
    true_df <- data.frame(Time = mids, alphas)
    true_df <- melt(true_df, id.vars = "Time")
    p1 <- p1 + geom_line(data = true_df, aes(x = Time, y = value,
                                             group = variable, 
                                             colour = variable), alpha = 1,
                         linetype = "dotted") 
  } 
  ...
  sampled.series <- data.frame(Time = bin.mids, t(alpha.post.pred.ord[ss,]))
  sampled.series <- melt(sampled.series, id.vars = "Time")
  
  p2 <-  ggplot(sampled.series) +
    geom_line(aes(x = Time, y = value,
                  group = variable, 
                  colour = variable)) + 
    scale_color_manual(values=colors[ss]) + 
    theme(legend.position = "none") +
    ylim(0,1) + 
    ylab("alpha") +
    xlab("Time (ms)")
  ...
  ## Average alpha.pred
  alpha_bar_POST <- rowMeans(alpha.post.pred)
  
  if(add.prior) {
    alpha_bar_PRIOR <- rowMeans(alpha.prior.pred)
    ...
    alpha_bar_df <- data.frame(Posterior = alpha_bar_POST)
    p3 <- ggplot(alpha_bar_df, aes(Posterior)) + 
      geom_density(alpha = 1/5)
  }
  ...
  ## Minmax
  if (add.prior){
    df <- melt(data.frame(MinMax = minmax.post.pred,
                          Prior = minmax.prior.pred))
    p4 = ggplot(df, aes(value, 
                        group = variable,
                        colour = variable,
                        fill = variable)) + geom_density(alpha = 1/5)
  } else {
    df <- data.frame(MinMax = minmax.post.pred)
    p4 <- ggplot(df, aes(MinMax)) + geom_density(alpha = 1/5)
  }
  
  # plot lambdas
  lw.A <- nbins*apply(lambda_A_POST,2,quantile,.025)
  up.A <- nbins*apply(lambda_A_POST,2,quantile,.975)
  m.A <- nbins*apply(lambda_A_POST,2,mean)
  lw.B <- nbins*apply(lambda_B_POST,2,quantile,.025)
  up.B <- nbins*apply(lambda_B_POST,2,quantile,.975)
  m.B <- nbins*apply(lambda_B_POST,2,mean)
  # put in single data frame
  full_df <- data.frame(Time = rep(bin.mids, 2), 
                        Sound = c(rep("A", nbins), rep("B", nbins)), 
                        Mean = c(m.A, m.B),
                        low = c(lw.A, lw.B),
                        up = c(up.A, up.B))
  ...
  for(i in 1:length(swing.cuts)) {
    nswing.post.pred[[i]] <- apply(alpha.post.pred, 1, swing.counter, width = swing.cuts[i])
  }
  names(nswing.post.pred) <- swing.cuts
  
  if(add.prior){
    for(i in 1:length(swing.cuts)){ 
      nswing.prior.pred[[i]] <- apply(alpha.prior.pred, 1, swing.counter, width = swing.cuts[i])
    }
    names(nswing.prior.pred) <- swing.cuts
    prior.tables <- lapply(nswing.prior.pred, table)
    prior.pred.list <- list()
    for (i in 1:length(prior.tables)){
      curr.table <- prior.tables[[i]]
      curr.widthes <- as.numeric(names(curr.table))
      swing.dist <- swing.cuts[i]
      curr.matrix <- matrix(0, ncol = 3, nrow = length(curr.table))
      for (j in 1:length(curr.table)){
        curr.count <- curr.table[j]
        curr.switches <- curr.widthes[j]
        curr.matrix[j,] <- c(swing.dist, curr.switches, 
                             curr.count)
      }
      prior.pred.list[[i]] <- curr.matrix
    }
    full.matrix.prior <- do.call(rbind,prior.pred.list)
  }
  tables <- lapply(nswing.post.pred, table)
  post.pred.list <- list()
  for (i in 1:length(tables)){
    ...
    post.pred.list[[i]] <- curr.matrix
  }
  full.matrix <- do.call(rbind,post.pred.list)
  full.df <- data.frame(Swing = as.factor(full.matrix[,1]),
                        Count = full.matrix[,2],
                        Freq = full.matrix[,3])
  
  p6 <- ggplot(full.df, aes(x = Swing, y = Count)) + 
    geom_point(aes(size = Freq), alpha = .8) +
    scale_size_continuous(range = c(3, 20)) +
    ylim(-.25, max(full.df$Count)) +
    xlab("Swing Magnitude") +
    ylab("Number of Swings") +
    theme(legend.position = "none")
  if (add.prior){
    full.matrix <- cbind(full.matrix, "Posterior")
    full.matrix.prior <- cbind(full.matrix.prior, "Prior")
    full.matrix <- rbind(full.matrix, full.matrix.prior)
    full.df <- data.frame(Swing = as.factor(full.matrix[,1]),
                          Count = as.numeric(full.matrix[,2]),
                          Freq = as.numeric(full.matrix[,3]),
                          Distribution = as.factor(full.matrix[,4]))
    p6 <- ggplot(full.df, aes(x = Swing, y = Count,
                              colour = Distribution,
                              fill = Distribution)) + 
      geom_point(shape = 21, aes(size = Freq), alpha = .5) +
